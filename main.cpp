...
map<string,string> chessRequestMap;
...
bool checkChessFormat(string txt){//[0-9][a-z][A-Z]
    if (txt.length()>8){
        return false;
    }
    for(int i=0;i<txt.size();i++){
    ...
    while (1) {
        ...
        if(code=="CHES"){
            try {
				...
				if(database.isFriend(user->getUsername(), friendUserName)){
                    string chessRequest="STCH?"+user->getUsername();
					send(myFriend->getSocket(),chessRequest.c_str(),chessRequest.length(), 0);
					pthread_mutex_lock(&hashLock);
					chessRequestMap[user->getUsername()]=friendUserName;
					pthread_mutex_unlock(&hashLock);
                    continue;
                }             
				if(!database.isFriend(user->getUsername(), friendUserName)){
                ...
                cout<<"user"<<user->getUsername()<<"start chess with friend Fail"<<endl;
                send(user->getSocket(), "ERRO", 4, 0);
            }
		...
		if(code=="STCH"){
            ...
            string message="RCHE?FROM="+user->getUsername()+"&CODE="+messageContent;
            send(userToSend->getSocket(), message.c_str(), message.size(), 0);
            continue;
        }
        ...
        if(code=="ACCH"||code=="RECH"){//accept or deny the friend invatation
            string friendUserName=data.substr(10,data.size()-10);
            pthread_mutex_lock(&DBLock);
            if(!database.isUserNameExists(friendUserName)){
                send(user->getSocket(), "NUSR", 4, 0);
            ...
            pthread_mutex_unlock(&DBLock);
            pthread_mutex_lock(&hashLock);
            if(chessRequestMap[friendUserName]!=user->getUsername()){
                
                pthread_mutex_unlock(&hashLock);
                continue;
            }
            chessRequestMap[friendUserName]="";
            pthread_mutex_unlock(&hashLock);
            User *userWhoSendMeTheRequest=isUserOnline(friendUserName);
            if(userWhoSendMeTheRequest!=NULL){//send accept message
                if(code=="RECH"){
                    string message="CHRE?FROM="+user->getUsername();
                    send(userWhoSendMeTheRequest->getSocket(), message.c_str(), message.size(), 0);
                }else{
                    string message="CHAC?FROM="+user->getUsername();
                    send(userWhoSendMeTheRequest->getSocket(), message.c_str(), message.size(), 0);
                }
            }
...
int main(int argc, char*argv[])
{
	...
	int SERVER_PORT;
	if (argc < 2)
    {   fprintf(stderr, "Usage: Need port number\n");
	exit(10);
    }
    SERVER_PORT = atoi(argv[1]);	/* get the port number */
